...
At the time of final submission, the upstream DeltaSignal repository is
private. Its PR numbers, merge states, methods, results, and reproduction
commands are recorded here, but its code links are visible only to Reactome
maintainers. The LNG and WebsiteAngular contributions linked below are public.

| Repository | Contribution | Upstream status |
| --- | --- | --- |
| DeltaSignal | PR #1: initial TCGA pipeline and preservation of LNG `edge_type` | Merged in private upstream |
...
distinction are documented in the DeltaSignal PR #11 work product.
...
The full data preparation and scoring contract is in the DeltaSignal PR #11
work product.
